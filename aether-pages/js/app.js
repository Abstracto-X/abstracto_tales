...
let store;
function loadStore(){ try { const raw = LS.getItem("aether-pages-v1"); store = raw ? Object.assign(defaultStore(), JSON.parse(raw)) : defaultStore(); } catch(e){ store = defaultStore(); } if(!store.settings) store.settings = defaultStore().settings; }
function saveStore(){ try { LS.setItem("aether-pages-v1", JSON.stringify(store)); } catch(e){} }
loadStore();
const persona = () => Object.assign({}, PERSONA_ACCESS[store.personaId] || PERSONA_ACCESS.patron);
...
function topbar(){
  ...
  return `<header class="topbar">
    ...
    <button class="tb-btn" data-sheet="persona" aria-label="Account">${I.user}</button>
  </header>`;
}
...
function render(){
  ...
  const apply = ()=>{
    ...
    if (inStudio){ ensureStudioChrome(); }
  ...
  <div class="home-cols">
   ...
   <div>
    ...
    <div class="section">
      ...
      <div class="col-flex">
        ${storyCardWide(D.STORIES[3])}
        ${storyCardWide(D.STORIES[4])}
      </div>
...
VIEWS.vault = function(){
  ...
  <div class="card" style="display:flex;gap:11px;align-items:center;margin-top:8px"><span class="faint">${I.cog}</span><div style="flex:1"><div style="font-weight:600;font-size:.86rem">Experience as a different reader</div><div class="faint" style="font-size:.74rem">Demo switcher — preview every access state.</div></div><button class="btn sm" data-sheet="persona">Switch</button></div>
  `;
};
...
function sheetPersona(){
  ...
  <div class="card tinted" style="margin-bottom:14px;display:flex;gap:12px;align-items:center"><span style="width:42px;height:42px;border-radius:50%;background:var(--accent-soft);display:grid;place-items:center;color:var(--accent)">${I.user}</span><div style="flex:1"><div style="font-weight:600">${P.signedIn?store.email:"Guest"}</div><div class="faint" style="font-size:.76rem">${P.tier||"No active access"}</div></div></div>
  <div class="quicklinks" style="margin-bottom:16px"><a data-nav="/vault">${I.vault}<span>Vault</span><small>Manage access</small></a><a data-nav="/my-shelf">${I.shelf}<span>My Shelf</span><small>Your library</small></a><a data-sheet="settings">${I.aa}<span>Preferences</span><small>Reader</small></a></div>
  <div class="eyebrow" style="margin-bottom:9px">Experience as… <span class="faint" style="text-transform:none;letter-spacing:0">(demo switcher)</span></div>
  <div class="col-flex">${D.PERSONAS.map(p=>`<button class="row ${store.personaId===p.id?'now':''}" data-persona="${p.id}" style="${store.personaId===p.id?'':''}"><span class="ic-col" style="color:var(--accent)">${store.personaId===p.id?I.check:I.user}</span><span class="body"><span class="t"><span class="tt">${p.label}</span></span><span class="sub">${p.access==='member'?'Aether Member access':p.access==='archivist'?'Archivist Tier':p.access==='expired'?'Expired membership':p.access==='pending'?'Sync pending':p.access==='no-tier'?'Connected, no tier':'No access'}</span></span></button>`).join("")}</div>`;
}
...
function sheetRedeem(){
  ...
  <div class="card" style="margin-top:14px"><div class="eyebrow" style="margin-bottom:6px">Demo keys</div><div class="faint" style="font-size:.76rem;line-height:1.7"><div><span class="kbd">AETHER-ARC2-2026</span> — Arc II preview</div><div><span class="kbd">REVIEWER-2026</span> — reviewer liturgy</div><div><span class="kbd">WRONG-KEY-9999</span> — see an error</div></div></div>`;
}
function sheetConnectPatreon(){
  return `<span class="close-x" data-act="close-sheet">${I.x}</span><h2>Connect Patreon</h2><p class="sheet-sub">Linking Patreon lets us verify your membership and unlock your tiers. We read your tier — nothing else.</p>
  <div class="card" style="margin-bottom:14px"><div class="between"><div><div style="font-weight:600">What this does</div><div class="faint" style="font-size:.78rem">Unlocks every chapter your tier includes, automatically.</div></div>${I.vault}</div></div>
  <div class="col-flex" style="gap:9px"><button class="btn story block" data-act="connect-patreon-go">${I.vault}Continue with Patreon</button><button class="btn ghost block" data-sheet="redeem">${I.key}I have a key instead</button><a class="btn ghost block" data-nav="/support/wrong-account">${I.user}Wrong account?</a></div>
  <p class="faint" style="font-size:.72rem;margin-top:12px">In the live app this opens Patreon's secure OAuth. Here it's simulated for the concept demo.</p>`;
}
...
function connectPatreonGo(){
  closeSheet();
  if(store.personaId==="anon"||store.personaId==="no-access"||store.personaId==="no-tier"||store.personaId==="lapsed"){ store.personaId="patron"; }
  saveStore();
  const ret=store.pendingReturn;
  toast("Patreon connected","Your access is verified and unlocked.", {icon:"checkCirc", ms:5000, action: ret?{act:"return:"+ret, label:"Return to chapter"}:null});
  setTimeout(()=>{ render(); }, 300);
}
function redeemKey(code){
  code=(code||"").trim().toUpperCase();
  const err=document.getElementById("key-error");
  const valid={"AETHER-ARC2-2026":{label:"Arc II Preview · The Glass Orchard", storyId:"glass-orchard"},"REVIEWER-2026":{label:"Reviewer Liturgy · Ash Saints", storyId:"ash-saints"}};
  if(!code){ if(err)err.textContent=""; return; }
  if(code==="WRONG-KEY-9999"||!valid[code]){ if(err){err.style.color="var(--bad)";err.textContent="This doesn't look like a valid key. Check for typos.";} return false; }
  if(store.redeemedKeys.some(k=>k.code===code)){ if(err){err.style.color="var(--warn)";err.textContent="This key has already been redeemed.";} return false; }
  const info=valid[code];
  store.redeemedKeys.unshift({code, label:info.label, when:"just now", storyId:info.storyId});
  store.grantedKey=true;
  saveStore();
  const ret=store.pendingReturn;
  closeSheet();
  toast("Access granted", info.label, {icon:"key", ms:5500, action: ret?{act:"return:"+ret, label:"Return to chapter"}:null});
  setTimeout(render,300);
  return true;
}
...
function delegate(){
  document.addEventListener("click",(e)=>{
    ...
    if (t.dataset.act!=null){ handleAct(t.dataset.act, t); return; }
  });
  ...
  document.addEventListener("submit",(e)=>{
    ...
    if(f.dataset.redeemForm!=null){ e.preventDefault(); const v=f.querySelector("[name=key]").value; const ok=redeemKey(v); if(ok===false){ /* error shown */ } return; }
...
function handleAct(act, el){
  switch(act){
    ...
    case "resync": toast("Re-running sync","We'll let you know the moment it clears.",{icon:"sync",ms:3500}); setTimeout(()=>{ if(store.personaId==="pending"){ store.personaId="patron"; saveStore(); render(); toast("Sync complete","3 chapters unlocked.",{icon:"checkCirc"}); } },2200); break;
...
VIEWS.home = function(){
  ...
  ${announcement()}
  ${banner}
  <div class="area-switch" style="margin:0 0 14px"><button class="active">${I.book}Reader</button><button data-nav="/studio">${I.overview}Author Studio</button></div>
...
function init(){
  ...
  render();
  // welcome toast for first demo load
  if(!LS.getItem("aether-welcomed")){ LS.setItem("aether-welcomed","1"); setTimeout(()=>toast("Welcome to Aether Pages","Tap the account icon (top-right) to preview every access state.",{icon:"spark",ms:6500}),900); }
}
