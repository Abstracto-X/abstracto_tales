...
- **Galactic Operations Deck Composition:** The interactive viewer follows the `PLANS/` map concepts with explicit overlay safe zones: a top command bar, left World Inspector, right Navicomputer, lower-right radar/status card, compact HUD control stack, centered legend, and bottom ticker/clock footer. Each surface shares the same cyan glass token system and avoids covering the primary controls of adjacent surfaces.
...
- **HUD Synchronization:** Route changes update the Navicomputer summary, analysis accordions, compact radar/status card, active-routes footer count, and itinerary together. The footer clock is managed by a single interval that is cleared by `MapViewer.destroy()`, and dock trigger `aria-expanded` state is synchronized with visual open/closed state.
- **Panel Minimization:** The top-right Minimize Panels control hides all non-map overlays while leaving the command bar and restore control available. Theme and audio controls are lightweight visual preferences only; they add no media or third-party dependencies.
