...
Operational requirement: Google must be enabled in Supabase Authentication providers, and the app URL used by `signInWithOAuth(... redirectTo ...)` must be present in Supabase Auth redirect URL allow-list. The frontend now uses a query-based redirect target shaped like `subscription.html?sub_auth=google&sub_route=vault` instead of relying on a hash-only `#/vault` redirect, because Supabase OAuth providers may ignore or reorder hash fragments. The callback consumer accepts both PKCE `?code=...` URLs and implicit `#access_token=...` URLs, including nested SPA hashes such as `subscription.html#/vault#access_token=...`, then cleans transient token/code parameters back to `#/vault` after session restore.
